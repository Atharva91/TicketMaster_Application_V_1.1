...
st.sidebar.image("ticketmaster_logo.png", width=200)
pages = ["Introduction", "Dynamic Pricing Concepts", "Data Overview", "Pricing Analysis", "Predictive Modeling", "SQL Playground", "Suggestions"]
...
# Page 5: Predictive Modeling
    ...
    if st.button("Predict Optimal Price"):
        if min_price <= 0 or max_price <= 0:
            st.warning("⚠️ Please enter valid values greater than 0 for both Minimum and Maximum Price.")
        elif max_price <= min_price:
            st.warning("⚠️ Maximum Price should be greater than Minimum Price.")
        else:
            optimal_price = model.predict(custom_input_encoded)[0]
            st.success(f"### ✅ Recommended Optimal Price: ${optimal_price:.2f}")
            st.write("""
...
elif selected_page == "SQL Playground":
    st.title("SQL Playground")

    st.write("### Data Schema")
    st.dataframe(data.head())

    st.markdown("ℹ️ **Table Name:** `df`")

    default_query = "SELECT City, AVG(\"Average Price $\") as Avg_Price FROM df GROUP BY City ORDER BY Avg_Price DESC"
    user_query = st.text_area("Write your SQL Query:", value=default_query, height=150)
